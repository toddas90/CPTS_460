...
int fbuf_init()
{
  ...
  ***************************************************************/
  fb = (int *)(3*1024*1024); // enough for 800x600 SVGA
  ...
  *(volatile unsigned int *)(0x10120010) = (3*1024*1024); // BaseAddress
...
int clrpix(int x, int y) // clear pixes at (x=col,y=row)
{
  int pix = y*640 + x;   // each row has 640 pixes=>pix = y*640 + x 
  fb[pix] = 0x00000000;
}

int setpix(int x, int y) // set pix at (x,y); color=BGR (reverse order or RGB)
{
  int pix = y*640 + x;
  if (color==RED)
     fb[pix] = 0x000000FF;
  if (color==BLUE)
    fb[pix] = 0x00FFFFFF;  // change to WHITE
  if (color==GREEN)
     fb[pix] = 0x0000FF00;
  if (color==CYAN)
     fb[pix] = 0x00FFFF00;
...
int dchar(unsigned char c, int x, int y) // display char at (x,y)
{
  ...
  caddress = font + c*16;  // get font location of char in fonts bit map
