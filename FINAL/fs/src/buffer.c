...
/************************ buffer.c ***************************************
 5-24-06: buffer management STRANGE problem: add to dev_list destroys dev_list
 Checked many times and could not find the reason.
 Finally figured out WHY: in hd_init(), when reading MBR, blk=0, which loads a
 buf with blk=0 with valid data. When trying to read extended partitions, also 
 used blk=0 relative to the start_sector of that partition. HOWEVER, getblk()
 faithfully FOUND the (WRONG) buf with blk=0 in cache, which screws up 
 everything afterward. Once the call to hd_init() is skipped, the buffers work
 FINE.
 Algorithm: Simple_PV algorithm
****************************************************************************/
/****** FD and HD dev# in type.h NEEDED in buffer.c FS/mount.c *******
#define FD 2
#define HD 3
*********************************************************************/
/******************* in type.h **********************
...
int binit(int d)  // d=0 called by hd.init()
{
  int i;
  struct buf *bp;
  if (d)
  ...
  for (i=0; i<NBUF; i++){
    ...
    /*********************************************************
     buffer data area is in the 1MB of BUFFER_BASE = 6MB
    **********************************************************/
    // bp->buf = (char *)(BUFFER_BASE + i*1024);
    /**********************************
    if (i<2 || i>1022)
        printf("[%d %x]  ", i, bp->buf);
    *********************************/
...
{
  // *(int *)(running->res->paddress + y) = requests;
  // *(int *)(running->res->paddress + z) = hits;
   *(int *)(y) = requests;
   *(int *)(z) = hits;
   return NBUF;  
}
