 ...
 *************************************************************************/
 ...
 /**************
 if ((mode & 0170000)==0020000 || (mode & 0170000)==0060000){// special file
    device = oftp->inodeptr->INODE.i_block[0];
    major = (device & 0xFF00) >> 8;

    blk = oftp->offset/1024; 
    oftp->offset += 1024;
    return (*devsw[major].dev_read)(device, blk, buf, nbytes);
 }
 ******************/
 if ((mode & 0170000)==0020000){ // char special file
    device = oftp->inodeptr->INODE.i_block[0];
    major = (device & 0xFF00) >> 8;
    //printf("kread: major=%x ", major);
...
int myread(int fd, char *buf, int nbytes, int space)
{
  ...
  n = MIN(nbytes, size); // read at most size bytes
  t = 0;
  ...
  while (n > 0){
     ...
     if (lbk < 12){              /* direct block */
        blk = ip->INODE.i_block[lbk];
         ...
         bp = bread(ip->dev, (u32)ip->INODE.i_block[13]);
         up = (u32 *)bp->buf;

         dblk = up[lbk/256];
         brelse(bp);

         //get_block(ip->dev, (u32)dblk, buf12);   /* get dindirect */
         bp = bread(ip->dev, (u32)dblk);   /* get dindirect */
         up = (u32 *)bp->buf;
