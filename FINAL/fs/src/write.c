...
int kwrite(int fd, char *buf, int nbytes)
{
  char c, *cp;
  OFT *oftp;
  u32 blk, i, n, device, mode, major, dev;
  ...
  *************/
  ...
  if ((mode & 0170000)==0060000){ // block special
    ...
    if (blk > devtab[dev].size/BLKSIZE){
	    ...
	    ip->INODE.i_blocks += 2;
	     ...
	     if (b12==0){
	       //prints("no more data blocks\n\r");
             ...
             //bp = getblk(ip->dev,(u32)ip->INODE.i_block[12]);
             //up = (u32 *)bp->buf;
             getblk(ip->INODE.i_block[12], buf12);
             up = (u32 *)buf12;

             for (i=0; i<256; i++)       /* zero out the indirect block */
                  up[i] = 0;
	    
             //put_block(ip->dev, (u32)ip->INODE.i_block[12], buf12);
             //bwrite(bp);
             putblock(ip->INODE.i_block[12], buf12);
          }
 
	  get_block(ip->dev, (u32)ip->INODE.i_block[12], buf12);
	  // bp = bread(ip->dev, (u32)ip->INODE.i_block[12]);
          //up = (u32 *)bp->buf;
          up = (u32 *)buf12;
	  ...
	  // bwrite(bp);
	     ...
	     if (b13==0){
	       //prints("no more data blocks\n\r");
             ...
             for (i=0; i<256; i++)     /* zero out zone[13] block */
                 up[i] = 0;
             //put_block(ip->dev, (u32)ip->INODE.i_block[13], buf12);
             bwrite(bp);
         } 
         //get_block(ip->dev, (u32)ip->INODE.i_block[13], buf12);
         dbp = bread(ip->dev, (u32)ip->INODE.i_block[13]);
         dup = (u32 *)dbp->buf;
	     ...
	     if (dblk==0){
	       ...
	       up[i] = 0;  
            ...
            //put_block(ip->dev, (u32)dblk, buf);   /* zero out the d_block */
             bwrite(bp);
         ...
         //get_block(ip->dev, (u32)dblk, buf12);
         bp = bread(ip->dev, (u32)dblk);
         up = (u32 *)bp->buf;
	     ...
	     ip->INODE.i_blocks += 2;
             //put_block(ip->dev, (u32)dblk, buf12); /* update the d_block */ 
             bwrite(bp);
         }
     }
