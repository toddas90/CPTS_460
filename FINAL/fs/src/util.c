 ...
 **********************************/
 ...
 /*  10-29-8:
     KCW must NOT reset refCount to 1 because while loading
...
iput(MINODE *mip)
{
 ...
 /*********************** KCW : 10-31-08 ******************************
  ...
  Tried: do not dec refCount to 0 and invalid its dev, ino ==>
  cannot be got as FREE, nor can be found as in core.===>
  seems to work. but grep still display lines NOT containing the pattern
  Got to find out why.
 **********************************************************************/
...
u32 ksearch(MINODE *mip, char *name)
{
  u32 i, count, nlen; 
   char *cp, c;
   DIR *dp; 
   ...
   for (i=0; i<12; i++){ /* search direct blocks only */
        if (mip->INODE.i_block[i] == 0) 
           return 0;
        bp = bread(mip->dev, (u32)mip->INODE.i_block[i]);
...
int findmyname(MINODE *parent, u32 myino, char *myname)
{
 ...
 for (i=0; i<12; i++){ /* search direct blocks only */
     ...
     //get_block(mip->dev, (u32)mip->INODE.i_block[i], buf);
...
u32 findino(MINODE *mip, u32 *myino)
{
  char *cp;   
  DIR  *dp;
  struct buf *bp;
  ...
  //get_block(mip->dev, (u32)mip->INODE.i_block[0], buf);
  bp = bread(mip->dev, (u32)mip->INODE.i_block[0]);
  cp = bp->buf; 
  dp = (DIR *)bp->buf;
...
{
  u32 i, ino, me, myparent, olddev, newdev, acc, nnames;
