...
int read_pipe(int fd, char *buf, int n)
{
  ...
  while(n){
    ...
    if (ret){   /* has read something */
     ...
     // pipe has no data
    if (p->nwriter>0){ // still has writer
       kwakeup(&p->room);
       ksleep(&p->data);
       continue;
    }
...
int kpipe(int *y, int *z)
{
  ...
  //  put_uword(pd[0], y);  put_uword(pd[1], y+1);
...
int close_pipe(int fd)
{
  ...
  if (op->mode == READ_PIPE){
      ...
      // printf("proc %d close_pipe: fd=%d nr=%d ref=%d\n",
      // running->pid, fd, pp->nreader, op->refCount);
  ...
  if (op->mode == WRITE_PIPE){
      ...
      //  printf("proc %d close_pipe: fd=%d nw=%d ref=%d\n",
      // running->pid, fd, pp->nwriter, op->refCount);
